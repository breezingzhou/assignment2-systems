...
=== Note:
+ `ds = p * (dp - delta[:, None]) * scale` 中的 `delta[:, None]` 不能写成 `delta`，否则广播会出错
+ `tl.atomic_add` 与 `普通变量累加 + 最后写入` 的区别
  - #table(
      ...
      [维度], [tl.atomic_add], [普通变量累加 + 最后写入],
      [并发安全性],
      [线程安全：多个线程同时操作同一地址时，保证累加操作 “不可分割”，结果正确],
      [线程不安全：多线程并发时会出现竞态条件，结果错误],

      [执行过程],
      [读取 - 修改 - 写入是一个原子步骤，中间不会被其他线程打断],
      [读取、修改、写入是三个独立步骤，中间可能被其他线程打断],

      [性能],
      [有轻微的原子操作开销（需要硬件锁 / 内存栅栏），但能保证正确性],
      [无原子开销，但并发场景下结果错误，无实际意义],

      [适用场景],
      [多线程需要并发修改同一内存地址（如全局计数器、直方图统计）],
      [每个线程操作独立的内存地址（无并发冲突）],
    )
