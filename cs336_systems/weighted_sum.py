...
def weighted_sum_fwd(
  ...
  for i in range(tl.cdiv(D, D_TILE_SIZE)):
    ...
    output = output + tl.sum(row * weight[None, :], axis=1)
...
def weighted_sum_backward(
    ...
    grad_output_ptr,  # Gradinput
    grad_x_ptr, partial_grad_weight_ptr,  # Gradoutputs
    stride_xr, stride_xd,
    stride_wd,
    stride_gr,
  ...
  for i in range(tl.cdiv(D, D_TILE_SIZE)):
    grad_output = tl.load(grad_output_block_ptr, boundary_check=(0,),
                          padding_option="zero")  # (ROWS_TILE_SIZE,)
    ...
    row = tl.load(x_block_ptr, boundary_check=(0, 1),
                  padding_option="zero")  # (ROWS_TILE_SIZE, D_TILE_SIZE)
...
class WeightedSumFunc(torch.autograd.Function):
  @staticmethod
  def forward(ctx: torch.autograd.function.BackwardCFunction, x: Tensor, weight: Tensor):
    ...
    ctx.D_TILE_SIZE = triton.next_power_of_2(D) // 16  # type: ignore
    ctx.ROWS_TILE_SIZE = 16  # type: ignore  # Each thread processes 16 batch elements at a time
  ...
  def backward(ctx, grad_out: Tensor):
    ...
    assert isinstance(ROWS_TILE_SIZE, tl.constexpr)
    assert isinstance(D_TILE_SIZE, tl.constexpr)
    n_rows, D = x.shape
    ...
    partial_grad_weight = torch.empty(
        size=(triton.cdiv(n_rows, ROWS_TILE_SIZE), D), device=x.device, dtype=x.dtype)  # type: ignore
